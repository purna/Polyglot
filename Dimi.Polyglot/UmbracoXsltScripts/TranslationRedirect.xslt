...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    ...
    <script language="javascript">
      location='<xsl:value-of select="concat(umbraco.library:NiceUrl($parentId), '?lang=', $currentPage/language)" />';
    </script>
    Please follow
    <a href="{concat(umbraco.library:NiceUrl($parentId), '?lang=', $currentPage/language)}">this link</a>
  </xsl:template>
</xsl:stylesheet>