  ...
  exclude-result-prefixes="msxml umbraco.library Exslt.ExsltCommon Exslt.ExsltDatesAndTimes Exslt.ExsltMath Exslt.ExsltRegularExpressions Exslt.ExsltStrings Exslt.ExsltSets ">
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="resultValue">
            ...
            <xsl:choose>
                <xsl:when test="$currentPage/*[name() = concat($currentPageTypeAlias, '_TranslationFolder')]/*[name() = concat($currentPageTypeAlias, '_Translation') and language = $langISO]/* [name() = $Property and not(@isDoc)] != '' and
string-length($currentPage/*[name() = concat($currentPageTypeAlias, '_TranslationFolder')]/*[name() = concat($currentPageTypeAlias, '_Translation') and language = $langISO]/* [name() = $Property and not(@isDoc)]) != 0">
                    <xsl:value-of disable-output-escaping="yes" select="$currentPage/*[name() = concat($currentPageTypeAlias, '_TranslationFolder')]/*[name() = concat($currentPageTypeAlias, '_Translation') and language = $langISO]/* [name() = $Property and not(@isDoc)]"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:value-of disable-output-escaping="yes" select="$currentPage/*[name() = concat($Property, '_', $langISO) and not(@isDoc)]"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:otherwise>
            </xsl:choose>
